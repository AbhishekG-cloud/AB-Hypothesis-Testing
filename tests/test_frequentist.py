...
def test_z_statistic_and_pvalue():
    ...
    my_result = two_proportion_ztest(n_A=1000, conv_A=100, n_B=1000, conv_B=120)

    z_ref, p_ref = proportions_ztest(count=[100, 120], nobs=[1000, 1000])

    assert np.isclose(abs(my_result["z_statistic"]), abs(z_ref))

    assert np.isclose(my_result["p_value"], p_ref)


def test_equal_conversion_rates():
    my_result = two_proportion_ztest(n_A=1000, conv_A=100, n_B=1000, conv_B=100)
...
def test_large_uplift():
    my_result = two_proportion_ztest(n_A=1000, conv_A=100, n_B=1000, conv_B=150)
...
def test_invalid_visitors():
    with pytest.raises(ValueError):
        two_proportion_ztest(n_A=-1000, conv_A=100, n_B=1000, conv_B=150)
...
def test_invalid_alpha():
    with pytest.raises(ValueError):
        two_proportion_ztest(n_A=-1000, conv_A=100, n_B=1000, conv_B=150, alpha=-0.05)
...
def test_smaller_mde_requires_more_samples():
    ...
    # Arrange
    sample_small_mde = minimum_sample_size(baseline_rate=0.10, mde=0.01)

    sample_large_mde = minimum_sample_size(baseline_rate=0.10, mde=0.05)
...
def test_higher_power_requires_more_samples():
    sample_small_power = minimum_sample_size(baseline_rate=0.10, mde=0.05, power=0.5)
...
def test_smaller_alpha_requires_more_samples():
    sample_small_alpha = minimum_sample_size(baseline_rate=0.10, mde=0.01, alpha=0.01)

    sample_large_alpha = minimum_sample_size(baseline_rate=0.10, mde=0.05)
...
def test_invalid_power():
    with pytest.raises(ValueError):
        sample_mde = minimum_sample_size(baseline_rate=0.10, mde=0.01, power=-0.9)
